...
知识库并不替代课程讲稿：`LectureNotes/` 提供课程教学来源，`docs/cards/` 把概念拆成可检索和练习的卡片，`docs/curriculum/` 负责把卡片重新组合为 21 讲教学序列。讲稿编号是来源锚点，不表示卡片必须与讲次一一对应。
...
- 若讲次安排发生变化，优先更新 `docs/curriculum/`；不要为匹配新顺序批量重命名只读讲稿。
