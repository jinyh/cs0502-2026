...
- 哈希把任意长度输入映射为固定长度摘要，不能“解密”。只有当预期摘要来自可信渠道时，对比摘要才能检测对抗性篡改。
- 数字签名用私钥签名、公钥验证，提供来源认证和完整性证据；验证者还必须通过证书或其他机制确认公钥确实属于声称的主体。
...
- 单独从同一下载页面取得“文件 + 哈希值”不能抵御该页面被一起篡改；公钥未经身份绑定也不能证明现实身份。
