...
- 2023+：开源模型（LLaMA 等）、多模态（GPT-4V）、Agent、推理模型（o1 类）。
...
性能随三要素协同增长：参数量、数据量、算力（Chinchilla 定律建议数据:参数约 20:1）。规模化不是「调大」，三者失衡会浪费或欠拟合。
...
- RAG：不更新参数，检索外部知识拼进上下文——事实性与可溯源性更好，是医疗/法律场景的首选。
...
根因：统计续写 ≠ 事实检索。模型学的是「什么话接下来像」，不是「什么话为真」。缓解：RAG、工具调用、采样温度调低、自校验、对齐。
...
LLM 是黑箱。机械可解释性（mechanistic interpretability）尝试逆向定位「电路」与特征，但远未成熟。
...
多个角色化 Agent 协作（如辩论、分工），提升复杂任务表现，也带来协调与成本问题。

### 5.4 推理模型

o1 类模型把「思考过程」显式化为长链 token，在数学/编程等需推理的任务上大幅提升，代价是更高推理算力。
...
- **训练数据枯竭**：高质量人类文本将耗尽，合成数据/self-improvement 受关注。
...
- **认知科学**：预测编码、注意力的神经对应。
- **医学**：医学问答 RAG、病历摘要、文献综述——幻觉风险高，须人工复核与检索锚定。
