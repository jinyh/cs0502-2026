...
$$Q^\pi(s,a)=\mathbb{E}\left[r+\gamma\,Q^\pi(s',\pi(s'))\right]$$
...
Deep Q-Network（DQN, 2015）用神经网络估 $Q$，靠经验回放与目标网络稳定训练，玩 Atari 超人。
...
限制策略更新步长，避免一次崩坏。简单稳定，是工业界默认算法，也是 RLHF 的主力。
...
RLHF 让「人类偏好」成为可优化的奖励信号——是 ChatGPT 成功的关键。详见 [`llm-deep-dive.md`](llm-deep-dive.md) 第 3.2 节。
