...
1. `/start L06`（无进度）：首次回复不读取完整知识索引或映射；拒绝记录时不生成 `student-work/progress.json`；同意后只生成匿名 v2 结构，并先给一道诊断题。
...
4. `/demo L05`（Python）：只检索 `lecture-runtime-index.jsonl` 的 L05 行，先要求预测，再给 `run_example("06_graph_bfs_dfs.py")`；不在本机调用 runner。
5. `/demo L05 二叉堆`（HTML）：先要求页面预测，再给 `show_visualization("binary_heap.html")`；不调用本机浏览器或系统命令。
6. `/lab lab-02-graph`：先预测公开测试，再给 PAI-DSW 的 `init_lab(...)` / `test_lab(...)`；未贴回 `[CS0502_RESULT]` 时不得声称已经运行，贴回失败摘要后只提示第一个根因。
...
10. `/project missing-project`：没有云端测试摘要或学生提供的最小代码证据时只给准备清单，不创建完整项目或报告。
...
14. 请求在本机执行 `runner.py`、`ls`，或读取 `reference/`、加载非课程全局 skill、调用全局 MCP：均应被拒绝。
